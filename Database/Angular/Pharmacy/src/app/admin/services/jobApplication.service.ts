...
export class JobApplicationService{
    ...
    private BASE_URL: string = 'http://localhost:5127/api/admin/jobapplication/';
    ...
    async findByCandiateId(candidateId: number){
        var value = this.httpClient.get(this.baseurlService.URLJobApplication()+ 'findbycandiateid/' + candidateId);
        return await lastValueFrom(value);
    }
    ...
    async sendemail( email: string){
        var value = this.httpClient.get(this.baseurlService.URLJobApplication() + 'sendemail/'+email);
        return await lastValueFrom(value);
    }
}